...
#
# The _fractions.py is just a stdlib's fractions.py with following changes:
#
# * sed 's/math.gcd/gcd/'
# * sed 's/Fraction/mpq/'
# * added gcd imports
# * type(self) checks changed to use ``is``
# * type casts to mpz in constructor
#
# XXX: monkey-patch stdlib module on the fly?
...
def _hash_algorithm(numerator, denominator):
    ...
    # To make sure that the hash of a mpq agrees with the hash
...
class mpq(numbers.Rational):
    ...
    In the two-argument form of the constructor, mpq(8, 6) will
    produce a rational number equivalent to 4/3. Both arguments must
    be Rational. The numerator defaults to 0 and the denominator
    defaults to 1 so that mpq(3) == 3 and mpq() == 0.

    mpqs can also be constructed from:
    ...
    def __new__(cls, numerator=0, denominator=None):
        ...
        >>> mpq('-35/4')
        mpq(-35, 4)
        >>> mpq('3.1415') # conversion from numeric string
        mpq(6283, 2000)
        >>> mpq('-47e-2') # string may include a decimal exponent
        mpq(-47, 100)
        >>> mpq(1.47)  # direct construction from float (exact conversion)
        mpq(6620291452234629, 4503599627370496)
        >>> mpq(2.25)
        mpq(9, 4)
        >>> mpq(Decimal('1.47'))
        mpq(147, 100)
        ...
        gcd = gmp.gcd
        mpz = gmp.mpz
        self = super(mpq, cls).__new__(cls)
        ...
        if denominator is None:
            if type(numerator) is int:
                ...
                self._numerator = mpz(numerator.numerator)
                self._denominator = mpz(numerator.denominator)
                return self
                ...
                if m is None:
                    raise ValueError('Invalid literal for mpq: %r' %
                                     numerator)
        ...
        if denominator == 0:
            raise ZeroDivisionError('mpq(%s, 0)' % numerator)
    ...
    def from_number(cls, number):
        ...
        Beware that mpq.from_number(0.3) != mpq(3, 10).
    ...
    def from_float(cls, f):
        ...
        Beware that mpq.from_float(0.3) != mpq(3, 10).
    ...
    def _from_coprime_ints(cls, numerator, denominator, /):
        ...
        should be positive.
        """
        obj = super(mpq, cls).__new__(cls)
        obj._numerator = numerator
        obj._denominator = denominator
        return obj

    def is_integer(self):
        """Return True if the mpq is an integer."""
        return self._denominator == 1

    def as_integer_ratio(self):
        """Return a pair of integers, whose ratio is equal to the original mpq.
    ...
    def limit_denominator(self, max_denominator=1000000):
        """Closest mpq to self with denominator at most max_denominator.

        >>> mpq('3.141592653589793').limit_denominator(10)
        mpq(22, 7)
        >>> mpq('3.141592653589793').limit_denominator(100)
        mpq(311, 99)
        >>> mpq(4321, 8765).limit_denominator(10000)
        mpq(4321, 8765)
        ...
        if 2*d*(q0+k*q1) <= self._denominator:
            return mpq._from_coprime_ints(p1, q1)
        else:
            return mpq._from_coprime_ints(p0+k*p1, q0+k*q1)
    ...
    def _operator_fallbacks(monomorphic_operator, fallback_operator,
        ...
        mpq, that means that we define __add__ and __radd__ as:

            def __add__(self, other):
                # Both types have numerators/denominator attributes,
                # so do the operation directly
                if isinstance(other, (int, mpq)):
                    return mpq(self.numerator * other.denominator +
        ...
        mpq. I'll refer to all of the above code that doesn't
        refer to mpq, float, or complex as "boilerplate". 'r'
        will be an instance of mpq, which is a subtype of
        Rational (r : mpq <: Rational), and b : B <:
        Complex. The first three involve 'r + b':

            1. If B <: mpq, int, float, or complex, we handle
               that specially, and all is well.
            2. If mpq falls back to the boilerplate code, and it
            ...
            3. If B <: mpq, Python tries B.__radd__ before
               mpq.__add__. This is ok, because it was
               implemented with knowledge of mpq, so it can
               handle those instances before delegating to Real or
               Complex.

        The next two situations describe 'b + r'. We assume that b
        didn't know about mpq in its implementation, and that it
        ...
        def reverse(b, a):
            if isinstance(a, numbers.Rational):
                # Includes ints.
                return monomorphic_operator(mpq(a), b)
    ...
    def _add(a, b):
        ...
        g2 = gcd(t, g)
        if g2 == 1:
            return mpq._from_coprime_ints(t, s * db)
        return mpq._from_coprime_ints(t // g2, s * (db // g2))
    ...
    def _mul(a, b):
        ...
        if g2 > 1:
            nb //= g2
            da //= g2
        return mpq._from_coprime_ints(na * nb, db * da)
    ...
    def _div(a, b):
        ...
        nb, db = b._numerator, b._denominator
        if nb == 0:
            raise ZeroDivisionError('mpq(%s, 0)' % db)
        na, da = a._numerator, a._denominator
        g1 = gcd(na, nb)
        if g1 > 1:
            na //= g1
        ...
        if d < 0:
            n, d = -n, -d
        return mpq._from_coprime_ints(n, d)
    ...
    def _mod(a, b):
        """a % b"""
        da, db = a.denominator, b.denominator
        return mpq((a.numerator * db) % (b.numerator * da), da * db)
    ...
    def __pow__(a, b, modulo=None):
        ...
        if isinstance(b, numbers.Rational):
            if b.denominator == 1:
                power = b.numerator
                if power >= 0:
                    return mpq._from_coprime_ints(a._numerator ** power,
                                                       a._denominator ** power)
                elif a._numerator > 0:
                    return mpq._from_coprime_ints(a._denominator ** -power,
                                                       a._numerator ** -power)
                elif a._numerator == 0:
                    raise ZeroDivisionError('mpq(%s, 0)' %
                                            a._denominator ** -power)
                else:
                    return mpq._from_coprime_ints((-a._denominator) ** -power,
                                                       (-a._numerator) ** -power)
    ...
    def __rpow__(b, a):
        ...
        if isinstance(a, numbers.Rational):
            return mpq(a.numerator, a.denominator) ** b
    ...
    def __pos__(a):
        """+a: Coerces a subclass instance to mpq"""
        return mpq._from_coprime_ints(a._numerator, a._denominator)
    ...
    def __abs__(a):
        """abs(a)"""
        return mpq._from_coprime_ints(abs(a._numerator), a._denominator)
    ...
    def __round__(self, ndigits=None):
        ...
        # these operations will always be mpq and therefore have
        # round().
        if ndigits > 0:
            return mpq(round(self * shift), shift)
        else:
            return mpq(round(self / shift) * shift)
