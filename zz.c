...
zz_divmod_near(const zz_t *u, const zz_t *v, zz_t *q, zz_t *r)
{
    ...
    if (zz_div(u, v, q, r) || zz_init(&halfQ) || zz_rshift1(v, 1, &halfQ)) {
        /* LCOV_EXCL_START */
    err:
        zz_clear(q);
        zz_clear(r);
    ...
    if (cmp == unexpect) {
        ...
        if (zz_init(&one) || zz_from_i64(1, &one) || zz_add(q, &one, q)) {
            /* LCOV_EXCL_START */
            zz_clear(&one);
            goto err;
            /* LCOV_EXCL_STOP */
...
zz_pow(const zz_t *u, const zz_t *v, zz_t *w)
{
    ...
    if (u->size == 1 && u->digits[0] == 1) {
        if (u->negative) {
            return zz_from_i64(v->digits[0]%2 ? -1 : 1, w);
        }
        else {
            return zz_from_i64(1, w);
        }
...
zz_gcdext(const zz_t *u, const zz_t *v, zz_t *g, zz_t *s, zz_t *t)
{
    ...
    if (t) {
        ...
        if (zz_init(&tmp) || zz_mul(u, tmp_s, &tmp)
            || zz_sub(tmp_g, &tmp, &tmp) || zz_div(&tmp, v, &tmp, NULL)
...
zz_powm(const zz_t *u, const zz_t *v, const zz_t *w, zz_t *res)
{
    ...
    if (u->negative || u->size > w->size) {
        ...
        if (zz_init(&tmp) || zz_div(u, w, NULL, &tmp)
            || zz_copy(&tmp, &o1))
        {
            /* LCOV_EXCL_START */
            zz_clear(&tmp);
    ...
    if (w->size == 1 && w->digits[0] == 1) {
        ret = zz_from_i64(0, res);
    }
    else if (!v->size) {
        ret = zz_from_i64(1, res);
    }
    else if (!u->size) {
        ret = zz_from_i64(0, res);
    }
